...
public class RunRead extends Activity {
        ...
        try {
            if (id_chapter < book.size()) {
                ...
                if (size_chapters_w > id_word) {
                    ...
                    addBookmark = false;
                    phrase = strprint;
                    if (strprint.length() >= DEFAULT_SH_V) {
                        pause += (time * (strprint.length() / DEFAULT_SH_V)) - time;
    ...
    private double getCoefPersent(Book book) {
        ...
        for (Chapter cur_ch : book) {
            ...
            else if ((id_chapter == id_cur_chapter) && (id_chapter > 0) && (id_word == 0))
                countWords = col_words + id_word;
            id_cur_chapter++;
        }
